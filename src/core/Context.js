...
module.exports = class Context {
    ...
    float = (expression, name, noWrite) => {
        console.log('float', expression, name, noWrite)
        if (typeof expression === 'number') {
            if (!noWrite) this.writeLine(`${name} = ${expression}`)
            return new LPFloat({context: this, lpMod: this.lpMod, name, expression: name})
        }
        if (expression instanceof LPFloat) {
            if (name && !noWrite) this.writeLine(`${name} = ${expression}`)
            return expression
        }
    ...
    boolean = (expression, name) => {
        ...
        } else if (expression instanceof LPBoolean) {
            if (name) this.writeLine(`${name} = ${expression}`)
            // return expression
        }
    ...
    $script(script, ...params) {
        if (typeof script !== 'function') throw new Error(`Expected script to be typeof function!`)
        return script(...params)
    }

    // buildV2(sandbox = {}, script, context) {
    //     assert.ok(this.lpMod instanceof LPMod, 'Expected lpMod to be instanceof LPMod')
    //
    //     this._build = {}
    //     sandbox.globals = {}
    ...
    //     const $IF = (rhs) => ({
    //         write: () => rhs.writeLine(`If ${rhs.expression || rhs}`)
    //     })
    //     // function $IF(rhs) {
    //     //     return ({
    //     //         write: function () {this.writeLine(`If ${rhs.expression || rhs}`)}
    //     //     })
    //     // }
    //
